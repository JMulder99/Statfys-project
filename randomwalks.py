...
axs[0].scatter(np.arange(0, steps), number_outside, alpha=0.4, s=3, label='particles outside radius {}'.format(sunradius))
axs[0].set_xlabel('Time [iteration]')
axs[0].set_ylabel('Photons outside radius {}'.format(sunradius))
axs[0].set_title('Randomwalk of {} photons with {} steps'.format(N, steps))

# subplot 1 - end position of all particles
circle = plt.Circle((0, 0), sunradius, color='y', fill=False, alpha=1, label='sun at radius {}'.format(sunradius))
axs[1].add_artist(circle)
axs[1].scatter(end_pos[:, 0], end_pos[:, 1], s=1, alpha=0.1, edgecolor=None, c='k', label='photons')
